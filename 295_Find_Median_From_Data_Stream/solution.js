/*
    Approach 1: Simple Sorting. Sort nums everytime findMedian() is called
        TC: O(n log n) due to sorting
        SC: O(n) storing all of nums

    Approach 2: Insertion sort as we get numbers. Use binary search to find insert location.
        TC: O(n) + O(log n) = O(n)
        SC: O(n) still

    Approach 3: Use a minheap and maxheap
        TC: O(log n) Heap insertions and deletions take about O(log n) time.
        SC: O(n) linear space to hold input nums
 */
